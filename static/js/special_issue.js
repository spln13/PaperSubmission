...
nextPage = () => {
    ...
    page += 1;
    window.location.href = '/special_issues/' + page.toString() + '/'
}
...
const createSpecialIssueBox = (id, impactFactor, fullName, abbreviation, publisher, issn, ccfRanking) => {
    let mother_box = document.getElementById('special_issues');
    let box = document.createElement('tr');
    box.innerHTML = '<td>' + ccfRanking + '</td><td>' + abbreviation + '</td><td><a href="/special_issue/' + id + '/">' + fullName + '</td><td>' + issn + '</td><td>' + impactFactor + '</td><td>' + publisher + '</td>';
...
window.onload = () => {
    ...
    fetch(url_list, {
        ...
        .then(data => {
            ...
            const special_issue_list = data['list'];
            for (let i = 0; i < special_issue_list.length; i++) {
                createSpecialIssueBox(special_issue_list[i]['id'], special_issue_list[i]['impact_factor'], special_issue_list[i]['full_name'], special_issue_list[i]['abbreviation'], special_issue_list[i]['publisher'], special_issue_list[i]['issn'], special_issue_list[i]['ccf_ranking']);
            }
